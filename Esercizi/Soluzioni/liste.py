...
def tabellina(fattore: int, iterazioni:int = 10) -> list:
    risultato = [n for n in range(iterazioni + 1)]
    for i in range(len(risultato)):
        print(f"{fattore} x {risultato[i]} = {fattore * risultato[i]}")
        risultato[i] = risultato[i] * fattore
    return risultato


quattro = tabellina(4, 12)
print(quattro)
...
def aggiorna_numeri(n: int) -> None:
    numeri.insert(0, n) if n % 2 == 0 else numeri.append(n)
...
unione = zip(nomi, nazioni)
classifica = []
pos = 0
for nome, nazione in unione:
    classifica.append(f"{pos + 1}° posto - {nome} - {nazione}")
    pos += 1
# print(classifica)
...
for n in range(len(nomi)):
    classifica.append(f"{n + 1}° posto - {nomi[n]} - {nazioni[n]}")
